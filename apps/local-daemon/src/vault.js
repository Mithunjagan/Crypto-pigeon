...
import { validatePasswordStrength, wrapVmk, unwrapVmk, deriveSubkey } from './vault-keys.js';
...
async function setPlatformPermissions(dir, configPath, dbPath) {
...
export class Vault {
    ...
    getAttachmentKey() {
        if (!this.attachmentMetaKey)
            throw new Error('vault_locked');
        return this.attachmentMetaKey;
    }
    getLocalTokenKey() {
        if (!this.localTokenKey)
            throw new Error('vault_locked');
