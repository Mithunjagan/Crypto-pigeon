...
export class RelayClient {
    ...
    async getSessionToken(vault) {
        try {
            const db = vault.database();
            const row = db.prepare('SELECT access_token FROM relay_auth WHERE singleton=1').get();
    ...
    async saveSessionToken(vault, token) {
        const db = vault.database();
    ...
    async authenticate(vault, store) {
        ...
        const { sessionToken } = (await respRes.json());
        await this.saveSessionToken(vault, sessionToken);
        return sessionToken;
    }
    async request(vault, store, path, options = {}) {
    ...
    async applyAccess(username) {
        ...
        if (!res.ok)
            throw new Error(`Access request failed: ${res.statusText}`);
        return res.json();
    }
    async activateAccess(payload) {
