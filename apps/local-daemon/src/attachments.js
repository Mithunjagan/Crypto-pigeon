...
function makeAad(attachmentId, chunkIndex, totalChunkCount) {
...
export async function encryptAndUploadAttachment(vault, store, relayClient, plaintext, filename, mimeType) {
...
export async function downloadAndDecryptAttachment(vault, store, relayClient, manifest, messageId) {
    ...
    for (let i = 0; i < manifest.chunkCount; i++) {
        ...
        const { nonce: resNonceB64, ciphertext: resCiphertextB64 } = (await chunkRes.json());
    ...
    db.prepare(`INSERT INTO attachments (attachment_id, message_id, encrypted_local_path, file_name_encrypted, file_size, file_hash, file_key)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run([
...
export async function readDecryptedAttachment(vault, attachmentId) {
    const db = vault.database();
    const row = db.prepare('SELECT encrypted_local_path, file_name_encrypted FROM attachments WHERE attachment_id = ?').get([attachmentId]);
    if (!row)
